...
BEGIN
    ...
    -- IF WRITE LIKE THIS, WHEN EXCEPTION CAUSES, THEN OTHER STEPO ARE IGNORED
--    SELECT LOG_ID
--      INTO vEXISTS_ROW
--      FROM DUAL 
...
--        WHEN NO_DATA_FOUND THEN vEXISTS_ROW := NULL;
    ...
    ELSIF vEXISTS_ROW IS NOT NULL AND vSTATUS = 'START' THEN
        SELECT SUBSTR(ERR_MSG, INSTR(ERR_MSG, '(') + 1, INSTR(ERR_MSG, ')') - INSTR(ERR_MSG, '(') - 1)
          INTO vREPET_CNT
          FROM TBL_PRTC_LOG
